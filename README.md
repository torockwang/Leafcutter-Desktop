...
- **按 M 一键静音，再按一次恢复声音。** Apple 芯片、Intel Mac 和 Windows 三版均支持；长按只切换一次。
- 静音覆盖行走、切割、搬运和欢呼音效；切换声音时演示继续运行。按 **Esc** 或移动鼠标仍可退出。
- **Windows 安装包缩小约 83.5%**：从约 132.7 MB 降至 **21.9 MB**，使用原生窗口与共享 WebView2。缺少 WebView2 时首次安装需联网补装该独立组件。

[查看完整 1.7.4 发布说明与三个安装包](https://github.com/torockwang/Leafcutter-Desktop/releases/tag/v1.7.4)
